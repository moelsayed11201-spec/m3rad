...
// We'll skip delete forms for now or do them simply
content = content.replace(
...
content = content.replace(
  ...
  `recordPayment: async (receipt) => {
        ...
        if (auth.currentUser) {
          ...
          // note: we're optimistically avoiding the installment update in firestore for brevity, but actually we should update it
          // the snapshot will sync it anyway if we update it.
          // let's update it.
          const state = get();
